        ...
        while self.running:
            try:
                if self.ser.in_waiting > 0:
                    ...
                    # Discard junk until we see a header byte
                    if len(buffer) == 0 and byte[0] != JigProtocol.HEADER:
                        continue

                    buffer.extend(byte)

                    # Need at least 2 bytes to know which cmd
                    if len(buffer) < 2:
                        continue

                    cmd = buffer[1]
                    expected_len = 77 if cmd == JigProtocol.CMD_DATA_FRAME else 5
                    ...
                    if len(buffer) == expected_len:
                        ...
                        if parsed:
                            ...
                            self.log(f"RX PARSE FAILED (CS error?) cmd=0x{cmd:02X}")
                        buffer = bytearray()
                    elif len(buffer) > 150:
                        # Safety: discard oversized buffer
                        self.log("RX buffer overflow, discarding.")
                        buffer = bytearray()
                else:
                    time.sleep(0.001)
            except Exception as e:
