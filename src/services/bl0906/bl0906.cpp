 ...
 * All Rights Reserved
 *
 * File name: bl0906.cpp
 *
 * Description:
 *
 *
 * Last Changed By:  $Author: dungtv $
 ...
 * @param   uart_service: UART service của kênh cần set (NULL = dùng UART hiện
 * tại)
 * @retval  None
 */
...
void bl0906_set_channel_no_reset(uint8_t channel) {
  if (channel < 4) {
    current_channel = channel;
  }
}
 ...
 * @retval  None
 */
static void bl0906_bias_correction(uint8_t addr, float measurements,
                                   float correction) {
  uint8_t index;
  switch (addr) {
  case RMSOS_1:
...
bool bl0906_is_correction_complete_or_timeout(void) {
  if (clock_time_exceed_ms(current_correction_par.start_time_ms,
                           CURRENT_CORRECTION_TIMEOUT_MS)) {
    return true;
  }
  foreach (i, NUMBER_RL) {
...
static void bl0906_handle_current_rsp(uint8_t *par, uint8_t par_len) {
  ...
  // Tính toán dòng điện
  // Công thức: I = data * Vref / (12875 * Rl * active_gain_i)
  // Rl = 2 mOhm = 0.002 Ohm
  float active_gain_i = get_active_gain_i(current_channel);
  float ampere_value = (float)data * Vref / (12875.0f * Rl * active_gain_i);
  ...
  DBG_Bl0906_SEND_FLOAT(active_gain_i);
  DBG_BL0906_SEND_STR_INFO("\nBL0906: Current[");
  DBG_BL0906_SEND_INT(index);
  ...
  if (current_correction_par.is_running == true) {
    if (current_correction_par.complete_flag[index] == false) {
      if (measurement_values[current_channel].current[index] == 0) {
        ...
        if (measurement_values[current_channel].current[index] <=
            CURRENT_OFFSET_MA_MAX) {
  ...
  bl0906_update_energy(TYPE_CURRENT,
                       measurement_values[current_channel].current[index]);
  ...
  DBG_BL0906_SEND_STR(", ");
  DBG_BL0906_SEND_INT(
      (uint16_t)measurement_values[current_channel].current[index]);
}
...
static void bl0906_handle_voltage_rsp(uint8_t *par, uint8_t par_len) {
  ...
  float voltage_calc =
      (float)data * Vref * (Rf + Rv) / (13162.0f * Rv * Gain_v * 1000.0f);
  ...
  bl0906_update_energy(TYPE_VOLTAGE,
                       measurement_values[current_channel].voltage);
  DBG_BL0906_SEND_STR("\n __VOLTAGE__:");
...
static void bl0906_handle_active_power_rsp(uint8_t *par, uint8_t par_len) {
  ...
  if (sign_bit == 1) {
    ...
    power_value = (float)data * Vref * Vref * (Rf + Rv) /
                  (40.4125 * Rl * Rv * active_gain_i * Gain_v * 1000);
  }
  ...
  bl0906_update_energy(TYPE_ACTIVE_POWER,
                       measurement_values[current_channel].active_power[index]);
  DBG_BL0906_SEND_STR("\n __ACTIVE_POWER__:");
  DBG_BL0906_SEND_BYTE(index);
  DBG_BL0906_SEND_STR(", ");
  DBG_BL0906_SEND_INT(
      (uint16_t)measurement_values[current_channel].active_power[index]);
  DBG_BL0906_SEND_STR(", ");
  DBG_Bl0906_SEND_FLOAT(
      measurement_values[current_channel].active_power[index]);
}
...
static void bl0906_handle_temperature_rsp(uint8_t *par, uint8_t par_len) {
  ...
  measurement_values[current_channel].temperature =
      (data - 64) * 12.5 / 59 - 40;
  bl0906_update_energy(TYPE_TEMPERATURE,
                       measurement_values[current_channel].temperature);
}
 ...
 * @brief   Note, make sure that all relays are off, otherwise this process may
 * cause errors.
 * @param
 * @retval  None
 */
void bl0906_current_correction_proc(void) {
  ...
  foreach (i, NUMBER_RL) {
    if (current_correction_par.complete_flag[i] == false) {
      if (clock_time_exceed_ms(current_correction_par.retry_start_time_ms,
                               CURRENT_CORRECTION_RETRY_INTERVAl_MS) ||
          (current_correction_par.retry_start_time_ms == 0)) {
...
static void bl0906_set_gain_proc(void) {
  ...
  // Đọc gain hiện tại trực tiếp từ thanh ghi để đảm bảo chính xác
  ...
  if ((gain_par[current_channel].value & 0xFFFF00) != GAIN_1_DEFAULT_VALUE) {
    ...
    DBG_BL0906_SEND_STR_INFO("\nBL0906: Channel ");
    DBG_BL0906_SEND_INT(current_channel);
    DBG_BL0906_SEND_STR_INFO(" Gain is healthy (x16).");
  }
}
 ...
 * @retval  None
 */
uint8_t _culcCheckSum(uint8_t *tx_data, int tx_len, uint8_t *rx_data,
                      int rx_len) {
  uint8_t checksum = 0;
...
static bool bl0906_write_register(uint8_t address, uint32_t data,
  ...
  UartService *target_uart =
      (uart_service != NULL) ? uart_service : p_uart_service;
  ...
  // Write Register
  uint8_t tx_data[6] = {BL0906_WRITE_COMMAND, address, (uint8_t)(data),
                        (uint8_t)(data >> 8), (uint8_t)(data >> 16)};
...
static void bl0906_read_register(uint8_t address) {
  ...
  uint32_t timeout_ms =
      20; // Giảm từ 30ms xuống 20ms theo tài liệu (frame timeout max ~16ms)
 ...
 * @retval  None
 */
void bl0906_get_voltage(void) { bl0906_read_register(V_RMS); }
...
/**
 * @func    bl0906_get_gain
 * @brief   Đọc giá trị thanh ghi GAIN_1 hiện tại
 * @param   None
 * @retval  None
 */
void bl0906_get_gain(void) {
  // Read register 0x60 (GAIN_1) via standard blocking read
  bl0906_read_register(GAIN_1);
  // The handler bl0906_handle_gain_rsp will update gain_par
  measurement_values[current_channel].gain = gain_par[current_channel].value;
}
 ...
 * @retval  None
 */
void bl0906_get_temperature(void) { bl0906_read_register(TPS); }
