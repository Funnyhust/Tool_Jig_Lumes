...
void zero_detect_init(void) {
  ...
  // Trên STM32, nếu các pin có cùng interrupt number, chúng sẽ trigger cùng
  // nhau Cần kiểm tra xem có phải vấn đề này không
  if (pin1 == pin2 || pin1 == pin3 || pin1 == pin4 || pin2 == pin3 ||
      pin2 == pin4 || pin3 == pin4) {
...
//     // QUAN TRỌNG: Reset count trước mỗi lần đo (disable interrupt để tránh
//     race condition) noInterrupts(); for (int i = 0; i < 4; i++) {
//         zero_detect_count[i] = 0;
//     }
//     // Bắt đầu đo ngay sau khi reset (không có delay giữa reset và bắt đầu
//     đo) uint32_t time_start = micros(); uint32_t duration_us = 1000000;  // 1
//     giây interrupts();
...
//     // Copy count vào biến static để đảm bảo kết quả chính xác và có thể dùng
//     sau này

//     for (int i = 0; i < 4; i++) {
//         zero_detect_result[i] = zero_detect_count[i];
//     }
//     interrupts();  // QUAN TRỌNG: Phải enable interrupt lại, nếu không hệ
//     thống sẽ bị treo!
